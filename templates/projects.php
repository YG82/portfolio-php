    ...
    <?php foreach ($projects as $item): ?>
      <div class="project-card p-6">
        ...
        <div>
          ...
          <ul class="flex flex-wrap gap-3 mb-4">
            <?php foreach ($item['stack'] as $icon): ?>
              <li>
                <img class="stack-icon" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/<?= toSlug($icon) ?>/<?= toSlug($icon) ?>-original.svg" alt="<?= htmlspecialchars($icon) ?>" title="<?= htmlspecialchars($icon) ?>">
              </li>
            <?php endforeach; ?>
          </ul>
        </div>
        ...
        <div class="mt-4 text-right">
          <a href="<?= htmlspecialchars($item['link'] ?? '#') ?>" target="_blank" class="text-cyan-400 hover:text-cyan-100 transition duration-300 ease-in-out inline-flex items-center">
            Voir le projet
